...
class StatisticTxtWriterTest {

    @Test
    void test() throws IOException {
        var stat = new StatisticDto().countryCode("de").total(1500L).withPhoto(500L).withoutPhoto(1000L).photographers(20L);

        var outputMessage = new MockHttpOutputMessage();
        new StatisticTxtWriter().writeInternal(stat, outputMessage);

        var txt = outputMessage.getBodyAsString(StandardCharsets.UTF_8);
        var lines = txt.split("\n");
